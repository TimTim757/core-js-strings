...
function findLongestWord(sentence) {
  ...
  const logestWord = words.reduce((longest, currentWord) => {
...
function extractNameFromTemplate(value) {
  const prefixLength = 'Hello, '.length;
  const suffixLength = 1;
  return value.slice(prefixLength, value.length - suffixLength);
}
...
function encodeToRot13(str) {
  return str.replace(/[a-z]/gi, function rot13(char) {
